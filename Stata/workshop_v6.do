...
by rand, sort: tab adhr_b if visit == 0
...
stset maxVisit_s if visit == 0, failure(deathOverall)
sts graph, by(rand) risktable ylabel(0.7 (0.1) 1) xlabel(#8) xtitle(Visit) ///
 ytitle(Survival probability) title(Kaplan-Meier Curve showing survival among continuous adherers) legend(on)
restore
...
/*Note that we are only fitting this in the uncensored person-time*/
...
logit death visit c.visit#c.visit rand mi_bin niha_b hiserchol_b hisertrigly_b hiheart_b chf_b ap_b ic_b ///
	diur_b antihyp_b oralhyp_b cardiom_b anyqqs_b anystdep_b fveb_b vcd_b if cens_new ==0 [pweight = stabw_t] , cluster(simid)
...
/*Remember to restrict to only the uncensored person-time*/
/*Also remember to use the weights, as well as robust standard errors*/
logistic death visit c.visit#c.visit rand rand#c.visit rand#c.visit#c.visit mi_bin niha_b hiserchol_b hisertrigly_b hiheart_b chf_b ap_b ic_b ///
	diur_b antihyp_b oralhyp_b cardiom_b anyqqs_b anystdep_b fveb_b vcd_b if cens_new ==0 [pweight = stabw_t] , cluster(simid)

/*Now, we standardize as before*/


/* Create simulated data where everyone is treated*/
/* Create simulated data where everyone is untreated*/
