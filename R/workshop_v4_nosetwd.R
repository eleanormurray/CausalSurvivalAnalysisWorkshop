...
p2 <- ggplot(results, aes(x=visit, y=s))+
...
# Create variable that identifies perfect adherers (when 0)
placebo$perfectAdhr <- ave(placebo$cens_new, placebo$simID, FUN=sum)
...
placebo$maxVisit <- unlist(by(placebo, placebo$simID, FUN=function(subsi){
  ...
  if(subsi$perfectAdhr[1]==0){# If there is perfect adherence at all times to baseline adherence,
